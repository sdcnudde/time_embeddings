...
for i in range(0, len(dims)):
    for j in range(0, len(wdws)):
        ...
        dim = dims[i]
        wdw = wdws[j]
        model = gensim_cust.models.Word2Vec(sentences, dates, timewdw=wdw, sample=0, size=dim, min_count=1, sg=skipg, seed=42, hs=0, negative=5, iter=iter)
