...
def train_batch_sg(model, sentences, dates, alpha, _work):
    ...
    cdef np.uint32_t tokens[MAX_SENTENCE_LEN]
    cdef np.uint32_t reduced_windows[MAX_SENTENCE_LEN]
    cdef int sentence_idx[MAX_SENTENCE_LEN + 1]
    cdef np.uint32_t par2[MAX_SENTENCE_LEN]

    cdef np.uint64_t indexes_dates[MAX_SENTENCE_LEN]
    cdef np.uint32_t indexes_dates_days[MAX_SENTENCE_LEN]
    cdef np.float64_t mean_dates[MAX_SENTENCE_LEN]
    cdef int sentence_idx_dt[MAX_SENTENCE_LEN + 1]

    cdef int window = model.window

    cdef int i, lb, rb, d
    cdef int effective_words = 0, effective_sentences = 0, eff_words_dates = 0, eff_sents_dates = 0
    ...
    for sent, sent_dt in zip(sentences, dates):
        if not sent:
            continue  # ignore empty sentences; leave effective_sentences unchanged
        for token, dt in zip(sent, sent_dt):
            ...
            indexes_dates[eff_words_dates] = int(dt)
            indexes_dates_days[eff_words_dates] = int((int(dt)/86400))
            if hs:
        ...
        sm = 0
        for k in range(flag + 1, effective_words):
            sm = sm + (indexes_dates[k] - indexes_dates[k-1])
        ...
        eff_sents_dates += 1
        sentence_idx_dt[eff_sents_dates] = eff_words_dates
    ...
    with nogil:
        for sent_idx in range(effective_sentences):
            ...
            for i in range(idx_start, idx_end):
                rb = i + 1
                lb = i - 1
                while (rb < idx_end) & ((indexes_dates[rb] - indexes_dates[i]) <= 86400) & (indexes_dates_days[rb] == indexes_dates_days[i]):
                    if negative:
                        next_random = fast_sentence_sg_neg(negative, cum_table, cum_table_len, syn0, syn1neg, size, indexes[i], indexes[rb], _alpha, work, next_random, word_locks)
                    rb = rb + 1
                while (lb >= 0) & ((indexes_dates[i] - indexes_dates[lb]) <= 86400) & (indexes_dates_days[lb] == indexes_dates_days[i]):
                    if negative:
                        next_random = fast_sentence_sg_neg(negative, cum_table, cum_table_len, syn0, syn1neg, size, indexes[i], indexes[lb], _alpha, work, next_random, word_locks)
                    lb = lb - 1

    return effective_words


def train_batch_cbow(model, sentences, dates, alpha, _work, _neu1):
    cdef int hs = model.hs
    cdef int negative = model.negative
    cdef int sample = (model.sample != 0)
    ...
    for sent, sent_dt in zip(sentences, dates):
        ...
        if (effective_words - flag - 1) == 0:
            mean_dates[effective_sentences] = 0
        else:
            mean_dates[effective_sentences] = sm/(effective_words - flag - 1)
        # keep track of which words go into which sentence, so we don't train
        # across sentence boundaries.
        # indices of sentence number X are between <sentence_idx[X], sentence_idx[X])
        effective_sentences += 1
    ...
    with nogil:
        for sent_idx in range(effective_sentences):
            ...
            for i in range(idx_start, idx_end):
                rb = i + 1
                lb = i - 1
                while (rb < idx_end) & ((indexes_dates[rb] - indexes_dates[i]) <= mean_dates[sent_idx]):
                    rb = rb + 1
                while (lb >= 0) & ((indexes_dates[i] - indexes_dates[lb]) <= mean_dates[sent_idx]):
                    lb = lb - 1

                if hs:
                    fast_sentence_cbow_hs(points[i], codes[i], codelens, neu1, syn0, syn1, size, indexes, _alpha, work, i, lb, rb, cbow_mean, word_locks)
                if negative:
                    next_random = fast_sentence_cbow_neg(negative, cum_table, cum_table_len, codelens, neu1, syn0, syn1neg, size, indexes, _alpha, work, i, lb, rb, cbow_mean, next_random, word_locks)
